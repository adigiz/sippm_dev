            ...
            <ul class="navbar-nav my-lg-0">
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" data-toggle="dropdown" >
                        @if(\App\Profile::where('user_id', \Illuminate\Support\Facades\Auth::id())->exists())
                            @if(\App\Profile::where('user_id',\Illuminate\Support\Facades\Auth::id())->first()->avatar == NULL)
                                <img src="/uploads/avatar/default.jpg" alt="user" class="profile-pic m-r-5" />{{\App\Profile::where('user_id',\Illuminate\Support\Facades\Auth::id())->first()->name}}<span class="caret"></span></a>
                            @else
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <a class="nav-link waves-effect waves-dark" href="{{ route('gantiPassword') }}"><i class="mdi mdi-key" aria-hidden="true"></i>
                        Ganti Password</a>
                        <a class="nav-link waves-effect waves-dark" href="{{ route('users.logout') }}" aria-haspopup="true" aria-expanded="false"><i class="fa fa-sign-out" aria-hidden="true"></i>
                        Logout</a>
                    </ul>
                </li>
            </ul>
